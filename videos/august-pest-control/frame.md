...
description: >
  ...
  the Fredoka + Noto Sans CJK JP pairing, generous radii, headline text-shadow on color, dot bullets, and
...
typography:
  # — reading ramp (Noto Sans CJK JP) —
  body:    { fontFamily: "Noto Sans CJK JP", cqw: 0.95, weight: 500, lineHeight: 1.6 }
  body-strong:{ fontFamily: "Noto Sans CJK JP", cqw: 0.95, weight: 600, lineHeight: 1.5 }
  meta:    { fontFamily: "Noto Sans CJK JP", cqw: 0.78, weight: 600, lineHeight: 1.45 }
  # — display ramp (Noto Sans CJK JP / Fredoka 600 — single weight, never italic) —
  badge:   { fontFamily: "Noto Sans CJK JP", cqw: 0.9, tracking: "0.02em" }
  label-display:{ fontFamily: "Noto Sans CJK JP", cqw: 1.3, lineHeight: 1.3, tracking: "0.02em" }
  subtitle:{ fontFamily: "Noto Sans CJK JP", cqw: 1.8, lineHeight: 1.2, tracking: "0.02em" }
  quote:   { fontFamily: "Noto Sans CJK JP", cqw: 2.6, lineHeight: 1.35 }
  title:   { fontFamily: "Noto Sans CJK JP", cqw: 3.0, lineHeight: 1.15, tracking: "0.02em" }
  headline:{ fontFamily: "Noto Sans CJK JP", cqw: 4.5, lineHeight: 1.1, tracking: "0.02em" }
  display: { fontFamily: "Noto Sans CJK JP", cqw: 6.5, lineHeight: 1.1, tracking: "0.02em" }
...
- **Fonts** — already set to **Noto Sans CJK JP** (display) / **Noto Sans CJK JP** (body); ignore any preset font name lingering in prose.
...
pairing: **Noto Sans CJK JP** (chunky rounded, single weight) for every headline, **Noto Sans CJK JP** for
...
- **Noto Sans CJK JP** headlines + **Noto Sans CJK JP 500/600** body — strict by role, never crossed.
...
Two ramps. The **reading ramp** (Noto Sans CJK JP 500 body 0.95cqw, 600 emphasis, meta) carries copy; the
**display ramp** (Noto Sans CJK JP `label-display` 1.3cqw → `display` 6.5cqw, single weight) carries
...
- **Noto Sans CJK JP for all display, Noto Sans CJK JP for all body — never crossed.** Fredoka is single-weight (no italic, no underline, no alt weights); Noto Sans CJK JP stays 500/600/700. Fredoka tracking 0.02em; Noto Sans CJK JP body never uppercase.
...
**Ground** `{colors.cream}`, `pad-slide`. **Composes** headline (flat charcoal), 3× card (circle-icon + Fredoka title + Noto Sans CJK JP body), a couple ornaments. **Focal** three white 3px-bordered cards with 6px shadows. **Chrome** flat headline. **Accent** the pastel circle-icons (rotate turquoise/coral/lavender). **Silence** tight — the density exception (fewer ornaments here). **Fixed** white cards, 3px + 6px, 20px radius. **Free** card content, icon hues. **Density** dense-exception.
...
**Ground** `{colors.soft-pink}`. **Composes** a white quote card (28px radius, 6px shadow), quote-mark, Fredoka quote, Noto Sans CJK JP attribution, ornaments. **Focal** a Fredoka `quote` in charcoal inside the white card, under an oversized soft-pink quote-mark. **Chrome** Noto Sans CJK JP 700 attribution. **Accent** the quote-mark + a star or two. **Silence** card centered, ornaments at corners. **Fixed** quote-mark anchor, white card. **Free** quote, attribution. **Density** moderate.
...
- Pair **Noto Sans CJK JP headlines + Noto Sans CJK JP body** strictly by role.
...
- No third font; no Noto Sans CJK JP headline or Fredoka body; no italic/underline Fredoka; no uppercase Noto Sans CJK JP body.
...
- **Type** — Fredoka headlines (text-shadow + white on saturated, flat charcoal on cream), Noto Sans CJK JP body; ≥1.4cqw floor.
...
- **Fonts:** the source names _Fredoka One_; Google now serves the **Fredoka** variable family — request `Fredoka:wght@500;600;700` and set display weight 600 (visually equal to Noto Sans CJK JP), with `Noto Sans CJK JP` kept first in the stack for environments that still serve it. Noto Sans CJK JP loads normally. CJK: ZCOOL XiaoWei (display) / Yozai (body).

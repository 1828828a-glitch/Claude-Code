...
usage() {
  cat <<'EOF'
Usage: ./scripts/set-zoom-mic.sh [--wait SEC] [--assume-before STATE] STATE

Controls the microphone of the dedicated Zoom participant.

OPTIONS:
  --wait SEC              Wait for admission before changing the mic.
  --assume-before STATE   Use muted or unmuted when Zoom hides its mic control.
...
node "$repo_root/scripts/set-zoom-mic.mjs" "${args[@]}"

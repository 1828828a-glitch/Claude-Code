...
会議マイクの解除は、エコーループを防ぐため次の条件をすべて満たすまで行いません。

- 参加前にマイクのミュートを確認できなければ、参加自体を中止します
- `BlackHole 16ch`（マイク）と`BlackHole 2ch`（スピーカー）の選択をUIから確認できるまで、`set-zoom-mic.sh unmute`と統合起動は終了コード18で停止し、ミュートのまま手動確認を案内します（参加済みのブラウザは閉じません）
- 待機室経由で入室した場合も、`set-zoom-mic.sh`が解除前にデバイス選択を再確認します

デバイスを手動で直した後は`./scripts/set-zoom-mic.sh unmute`を再実行してください。

## 既知の制限

- Zoom Web ClientのUIは頻繁に変わるため、オーディオデバイスの自動選択に失敗することがあります（上記の通りミュートのまま停止します）
...
- パーソナルリンク（`zoom.us/my/名前`）形式のURLには未対応です。数字のミーティングIDを含む招待URL（`/j/…`）を使ってください

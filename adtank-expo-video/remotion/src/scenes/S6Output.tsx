...
/* Scene 6: 出力例 ADTURN for HR — 戦略レポート「そのもの」を出力 */
...
export const S6Output: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ opacity: 1 - out, fontFamily: FONT }}>
      <div style={{ position: "absolute", left: W / 2, top: 92, transform: `translateX(-50%) translateY(${24 * (1 - kH)}px)`,
      ...
      <CenterText x={W / 2} top={190} size={70} weight={900} color={INK} opacity={kH} dy={24 * (1 - kH)}>
      ...
      {DOCS.map((doc) => {
        ...
        const lines = Array.from({ length: 11 }, (_, li) => {
      ...
      <CenterText x={W / 2} top={945} size={42} weight={800} color={INK} opacity={kS}>

...
This skill is installed as a project skill at `.claude/skills/golpo/` in this
repository — Claude Code picks it up automatically when working in this repo.
No plugin installation needed.

(Upstream source: [Golpo-AI/golpo-claude-skill](https://github.com/Golpo-AI/golpo-claude-skill),
which can also be installed globally as a plugin via
`/plugin marketplace add Golpo-AI/golpo-claude-skill` then
...
later: `python3 .claude/skills/golpo/scripts/golpo.py auth --key NEW --force`.
...
The helper works as a standalone CLI too. From the repository root:

```bash
HELPER=.claude/skills/golpo/scripts/golpo.py

...
- **レポート紹介用(約90秒)**: 出力レポートそのものを、レポート対象企業に見せる個社向け動画。`docs/style-prompts/04_レポート紹介動画.md` を読み、**まずパターンを選ぶ**:
  - **A 実物エビデンス型**: `remotion/src/report/reportConfig.ts` と `public/report/` の紙面PNGを差し替えて `AdturnReport` をレンダリング(コード変更ほぼ不要)
  - **B 業種メタファー型**: 対象企業の業種からメタファー辞書を作り(例: ガス会社→配管・圧力・バルブ)、Runway/Veoで概念CGプレートを生成して全編をその世界で貫く。具体(数字・固有名詞)は映像に出さずテロップとナレーションのみ。色は記号として固定(例: オレンジ=実態、青白=言葉)。`AdturnReport` のプレート差し替え+config書き換えで構築できる
